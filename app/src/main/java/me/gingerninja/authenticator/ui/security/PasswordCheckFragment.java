...
public class PasswordCheckFragment extends BaseFragment<PasswordCheckFragmentBinding> {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        getViewModel(PasswordCheckViewModel.class).setUsePin(true); // TODO get from sharedprefs

        if (!hasLock()) {
            //getNavController().navigate(PasswordCheckFragmentDirections.openLockTypeSelectorAction(source));
        } else {
        ...
        if (event.handle()) {
            switch (event.getId()) {
                case PasswordCheckViewModel.EVENT_CONFIRM:
                    if (hasLock() && source == R.string.settings_security_bio_key) {
                        getNavController().navigate(PasswordCheckFragmentDirections.passwordCheckToBiometricsSetupAction());
                    } else {
                        getNavController().navigate(PasswordCheckFragmentDirections.openLockTypeSelectorAction(source));
                    }
                    break;
